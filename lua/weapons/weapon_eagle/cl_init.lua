...
language.Add("weapon_eagle", "Desert Eagle")
...
SWEP.WepSelectIcon = surface.GetTextureID("HUD/weapons/weapon_eagle") 
SWEP.BounceWeaponIcon = false 

local mat = Material("sprites/redglow1")
usermessage.Hook("HLR_deagle_laserdot_draw", function(um)
	...
	local iInvalidInc = 0
	hook.Add("HUDPaint", "HLR_deagle_laserdot_draw" .. iIndex, function()
		if !bValid then
			...
			if IsValid(ent) then bValid = true
				...
				if iInvalidInc >= 10 then
					hook.Remove("HUDPaint", "HLR_deagle_laserdot_draw" .. iIndex)
				end
				return
			end
		end
		...
		local size = math.Clamp((20 /fDist) *400, 0, 50)
		local tr = {}
